...
import { makrukgroundDests, scalachessCharPair } from './compat.js'
...
test('chessground dests with regular castle', () => {
  ...
  const wtm = Makruk.fromSetup(setup).unwrap()
  expect(makrukgroundDests(wtm).get('e1')!.sort()).toEqual(['a1', 'c1', 'd1', 'd2', 'e2', 'f1', 'f2', 'g1', 'h1'])
  expect(makrukgroundDests(wtm).get('e8')).toBeUndefined()

  setup.turn = 'black'
  const btm = Makruk.fromSetup(setup).unwrap()
  expect(makrukgroundDests(btm).get('e8')!.sort()).toEqual(['a8', 'c8', 'd7', 'd8', 'e7', 'f7', 'f8', 'g8', 'h8'])
  expect(makrukgroundDests(btm).get('e1')).toBeUndefined()
})
...
test('chessground dests with chess960 castle', () => {
  ...
  const pos = Makruk.fromSetup(setup).unwrap()
  expect(makrukgroundDests(pos).get('b8')!.sort()).toEqual(['a8', 'c8', 'e8'])
})
